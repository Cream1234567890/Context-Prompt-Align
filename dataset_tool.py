...
def fast_merge(node, label, split, dataset="Twibot-20"):
    assert dataset in dataset_names, f"Invalid dataset {dataset}"
    # dataset_dir = get_data_dir(server_id) / dataset
    ...
    node_info = node
    # node_info = pd.read_json("/root/BotRGCN/node.json")
    # label = pd.read_csv("/root/BotRGCN/label.csv")
    # split = pd.read_csv("/root/BotRGCN/split.csv")
