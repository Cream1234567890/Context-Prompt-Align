...
count=0
# 读取train.json文件
with open('/root/autodl-tmp/BotRGCN/twibot_20/fangxx/twbot20/new_data/newtest.json', 'r') as train_file:
...
for user_entry in train_data:
    uid = user_entry['ID']
    json_file_path = f'./processed_data/test_json_qlora/u{uid}.json'  # 替换为JSON文件的路径
    try:
        # 读取对应的JSON文件
...
output_file_path = "./new_data/newtest_qlora.json"  # 新的JSON文件路径
